...
//第二次结束(这是替换值的方式，指针变化的方式左右值不用更新,左节点始终是大值biggerindex始终是左，值替换就行)
//               17[0]
//       3[1]            8[2]
//   6[3]   7[4]  
...
//1、i=k=(lastIndex-1)/2得到的是所有含有子节点的节点[0]、[1]、[2]，循环得到这些节点。
...
//7、这样就把最大一个值放到最后了，而且这里还有一个作用，让最大值在下次循环二叉树中不参加排序。numbers[0]里原来是最大值
